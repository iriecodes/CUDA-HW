...
/*
 ...
 Also, note that it says "Async" at the end. That means the CPU tells the GPU to do the copy but doesn't wait around for it to finish.
	...
	GPU: 	"I might be all tied up watching a TikTok video of a guy eating hotdogs with his hands tied behind his back... underwater." // I like hodogs
 ...
 500
 You added the two vectors correctly on the GPU
 The time it took on the CPU was 0 microseconds
 The time it took on the GPU was 1314 microseconds

 1000
 You added the two vectors correctly on the GPU
 The time it took on the CPU was 0 microseconds
 The time it took on the GPU was 503 microseconds

 1500
 Something went wrong in the GPU vector addition
 1024 is the max # of threads within a block 
 AFTER WHILE LOOP FIX
  You added the two vectors correctly on the GPU
 The time it took on the CPU was 0 microseconds
 The time it took on the GPU was 502 microseconds
...
#define N 1500 // Length of the vector // max threads per block is 1024
...
{
	//BlockSize.x = N;
	BlockSize.x = 1024; // max threads per block
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	//wrapped in a loop to grab the vectors outside the blocks range
	while (id < n)
	{
		c[id] = a[id] + b[id];
		id += 1024; // skips ahead to the next set 
	}
