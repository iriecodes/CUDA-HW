...
/*
 ...
 To fix the code, I hard coded blockSize.x = 256. 
 Then, I used the formula from class to calculate GridSize.x = ((N-1)/BlockSize.x) + 1; In this case it would be (11503 - 1)/256 = [44.9296875 truncation (44)] + 1 = 45 
 	which is how many blocks is needed to process each element within the vector.
 Then to correct the indexing I used the formula from class int id = blockDim.x * blockIdx.x + threadIdx.x; which calculates the global thread id. 
