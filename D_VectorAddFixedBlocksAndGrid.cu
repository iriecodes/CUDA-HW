...
// Defines
#define N 11503//000 // Length of the vector
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	{
		#pragma unroll //reduces overhead by giving a hint to the compiler, unrolls the for loop into sequential steps
		for(int i = id; i<n; i+=blockDim.x*gridDim.x) // allows us to stride through entire array
		{
			//c[id] = a[id] + b[id];
			c[i] = sqrt(cos(a[i])*cos(a[i]) + a[i]*a[i] + sin(a[i])*sin(a[i]) - 1.0) + sqrt(cos(b[i])*cos(b[i]) + b[i]*b[i] + sin(b[i])*sin(b[i]) - 1.0);
		}
	}
}
