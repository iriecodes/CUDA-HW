...
__global__ void dotProductGPU(float *a, float *b, float *C_GPU, int n)
{
	...
	//handles multiplication part
	if (idx < n )
	{
		C_GPU[idx] = a[idx] * b[idx];
	}

	__syncthreads(); //WAITTTTT to finish multiplying
	...
	while (elements > 1)
	{
		...
		//if its odd, add left over piece to the front
		if (idx == 0 && elements % 2 == 1)
		{
			C_GPU[0] += C_GPU[elements - 1];
		}

		__syncthreads(); //WAAAIT for leftover addition

		if (idx < half)
		{
			C_GPU[idx] += C_GPU[idx + half];
		}

		__syncthreads(); //waittttt for fold

		elements = half; //weve been halved
	}	
}
