...
/*
 ...
 1. Make sure the number of threads per block is a power of 2. This avoids messy edge
	...
	if(BlockSize.x == 0 || (BlockSize.x & (BlockSize.x - 1)) != 0) //formula from class for bitshifting
	{
		printf("\n DUN DUN DUNNN: BLOCK_SIZE must be a power of 2, but it is %d.\n\n", BLOCK_SIZE);
		exit(0);
	}
 ...
 2. Calculate the correct number of blocks needed to process the entire vector.
	...
	//checls the block size is within what this GPU can handle
	if(BlockSize.x > bestProp.maxThreadsPerBlock || BlockSize.x > bestProp.maxThreadsDim[0])
	{
		printf("\n DUN DUN DUNNN: BLOCK_SIZE = %d is too big for this GPU (maxThreadsPerBlock = %d, maxThreadsDim[0] = %d).\n\n", BLOCK_SIZE, bestProp.maxThreadsPerBlock, bestProp.maxThreadsDim[0]);
		exit(0);
	}
	
	//checks gridsize is within gpu limits
	if(GridSize.x > bestProp.maxGridSize[0])
	{
		printf("\n DUN DUN DUNNN: This vector needs %d blocks but this GPU only allows %d blocks in the x dimension.\n\n", GridSize.x, bestProp.maxGridSize[0]);
		exit(0);
	}
 ...
 3. It’s inefficient to check inside your kernel if a thread is working past the end of the vector
	...
	//how many elements needed to fill entire grid
	int paddedN = GridSize.x * BlockSize.x;

	//allocate to padded length instead of N
	cudaMalloc(&A_GPU,paddedN*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
	//zeroing using cudaMemset
	...
	//C_GPU is a single float that every block atomically adds into
 ...
 4. In previous assignments, we had to do the final reduction on the CPU because we couldn't sync blocks.
	...
	__global__ void dotProductGPU(float *a, float *b, float *c, int n)
	{
		...
		while(1 < fold)
		{
			...
			/* No such thing as odd anymore therfore its not needed
			if(fold%2 != 0)
			{
		...
		if(threadIndex == 0)
		{
			atomicAdd(c, c_sh[0]); // atomic adding from shared to actual c vect
		}
	}

	This step occurs in int main()
	// Copy Memory from GPU to CPU	
	//cudaMemcpyAsync(C_CPU, C_GPU, N*sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpy(&DotGPU, C_GPU, sizeof(float), cudaMemcpyDeviceToHost);//only the finished dot prod is saved
	cudaErrorCheck(__FILE__, __LINE__);
	...
	//count gpus, find the best
	int deviceCount;
	cudaGetDeviceCount(&deviceCount);
	cudaErrorCheck(__FILE__, __LINE__);
	if(deviceCount == 0)
	{
		printf("\n DUN DUN DUNNN: No CUDA capable GPU was found.\n\n");
		exit(0);
	}
	...
	cudaDeviceProp prop;
	cudaGetDeviceProperties(&bestProp, 0);
	cudaErrorCheck(__FILE__, __LINE__);
	for(int i = 1; i < deviceCount; i++)
	{
		cudaGetDeviceProperties(&prop, i);
		cudaErrorCheck(__FILE__, __LINE__);
		if(prop.major > bestProp.major || (prop.major == bestProp.major && prop.minor > bestProp.minor))
		{
			bestDevice = i;
			bestProp = prop;
		}
	...
	cudaErrorCheck(__FILE__, __LINE__);
	printf("\n Using GPU %d: %s (compute capability  major:%d minor:%d)\n", bestDevice, bestProp.name, bestProp.major, bestProp.minor);
	
	//atomic adds on floats need compute capability 3.0 or higher
	if(bestProp.major < 3)
	{
		printf("\n DUN DUN DUNNN: Float atomic adds need compute capability 3.0 or higher");
		exit(0);
	}
...
Placed my answers in corresponding pregaming section. Comments also placed throughout new code.
...
int main()
{
	...
	/* no longer needed because of atomic add
	DotGPU = 0.0;
